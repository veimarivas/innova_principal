...
<script>
    (function() {
        ...
        window.OFERTA_ID = <?php echo e($oferta->id); ?>;
        window.CANTIDAD_SESIONES = <?php echo e($oferta->cantidad_sesiones); ?>;
        window.CSRF = CSRF;
    ...
    `;
        ...
        $('#btnBuscarDocenteModulo').on('click', function() {
            ...
            $.ajax({
                    url: '<?php echo e(route('admin.posgrads.modulos.buscar-docente')); ?>',
                    type: 'POST',
                    data: {
                        _token: CSRF,
                        carnet: carnet,
        ...
        $('#btnRegistrarYAsignarDocente').on('click', function() {
            ...
            $.ajax({
                    url: '<?php echo e(route('admin.posgrads.docentes.registrar')); ?>',
                    type: 'POST',
                    data: {
                        _token: CSRF,
        ...
        function initContable() {
            $.getJSON('<?php echo e(route('admin.posgrads.ofertas.planes-conceptos.listar', $oferta->id)); ?>').done(function(
                r) {
                contableData = r.data || [];
                renderContableCards();
            }).fail(function() {
        ...
        function autoCompletarPrecio(filaIdx) {
            ...
            const ruta =
                '<?php echo e(route('admin.posgrads.ofertas.planes-conceptos.precio-base', ['ofertaId' => $oferta->id, 'conceptoId' => '__ID__'])); ?>'
        ...
        $('#btn-nuevo-plan-concepto').on('click', function() {
            ...
            $.when($.getJSON('<?php echo e(route('admin.posgrads.ofertas.planes-pago.disponibles', $oferta->id)); ?>'),
                    $.getJSON('<?php echo e(route('admin.posgrads.ofertas.conceptos.disponibles', $oferta->id)); ?>'), $
                    .getJSON(
                        '<?php echo e(route('admin.posgrads.ofertas.planes-conceptos.verificar-principal', $oferta->id)); ?>'
                    ))
        ...
        $('#btnGuardarPc').on('click', function() {
            ...
            $.ajax({
                    url: '<?php echo e(route('admin.posgrads.ofertas.planes-conceptos.multiple', $oferta->id)); ?>',
                    type: 'POST',
                    data: {
                        _token: CSRF,
                        planes_pago_id: planId,
...
// Función para cargar el detalle de un plan específico
        ...
        (function() {
            ...
            var resumenPorConcepto = <?php echo json_encode($resumenPorConcepto ?? [], 15, 512) ?>;
...
(function() {
    ...
    const OFERTA_ID   = <?php echo e($oferta->id); ?>;
    ...
    const OFERTA_DATA = {
        codigo:                     '<?php echo e(addslashes($oferta->codigo)); ?>',
        posgrado_id:                '<?php echo e($oferta->posgrado_id); ?>',
        programa_id:                '<?php echo e($oferta->programa_id); ?>',
        fase_id:                    '<?php echo e($oferta->fase_id); ?>',
        sucursale_id:               '<?php echo e($oferta->sucursale_id ?? ''); ?>',
        modalidade_id:              '<?php echo e($oferta->modalidade_id ?? ''); ?>',
        fecha_inicio_inscripciones: '<?php echo e($oferta->fecha_inicio_inscripciones ? $oferta->fecha_inicio_inscripciones->format('Y-m-d') : ''); ?>',
        fecha_inicio_programa:      '<?php echo e($oferta->fecha_inicio_programa ? $oferta->fecha_inicio_programa->format('Y-m-d') : ''); ?>',
        fecha_fin_programa:         '<?php echo e($oferta->fecha_fin_programa ? $oferta->fecha_fin_programa->format('Y-m-d') : ''); ?>',
        gestion:                    '<?php echo e($oferta->gestion); ?>',
        n_modulos:                  '<?php echo e($oferta->n_modulos); ?>',
        cantidad_sesiones:          '<?php echo e($oferta->cantidad_sesiones); ?>',
        version:                    '<?php echo e($oferta->version); ?>',
        grupo:                      '<?php echo e($oferta->grupo); ?>',
        nota_minima:                '<?php echo e($oferta->nota_minima); ?>',
        color:                      '<?php echo e($oferta->color ?? '#fc7b04'); ?>',
        responsable_academico_id:   '<?php echo e($oferta->responsable_academico_id ?? ''); ?>',
        responsable_marketing_id:   '<?php echo e($oferta->responsable_marketing_id ?? ''); ?>',
    };
    ...
    window.abrirModalDocSolo = function(tipo) {
        ...
        if (esPortada) {
            estadoEl.innerHTML = '<i class="ri-checkbox-circle-fill" style="color:#16a34a;"></i> <a href="<?php echo e(asset('storage/' . $oferta->portada)); ?>" target="_blank" style="color:#16a34a;font-size:.68rem;text-decoration:underline;">ver actual</a>';
        }
        <?php else: ?>
        ...
        if (!esPortada) {
            estadoEl.innerHTML = '<i class="ri-checkbox-circle-fill" style="color:#16a34a;"></i> <a href="<?php echo e(asset('storage/' . $oferta->certificado)); ?>" target="_blank" style="color:#16a34a;font-size:.68rem;text-decoration:underline;">ver actual</a>';
        }
        <?php else: ?>
...
<?php /**PATH C:\xampp_82_12\htdocs\innova-ciencia-virtual\resources\views/admin/ofertas-academicas/partials/ofertas-detalle/scripts.blade.php ENDPATH**/ ?>