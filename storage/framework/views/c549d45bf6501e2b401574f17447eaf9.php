<?php $__env->startSection('title'); ?> Grados Académicos <?php $__env->stopSection(); ?>
        ...
        <div class="dph-inner">
            <div class="dph-left">
                ...
                <div class="dph-text-block">
                    <h1 class="dph-title">Grados Académicos</h1>
                    <p class="dph-desc">Gestión y administración de grados académicos</p>
                </div>
            </div>
            <div class="dph-right">
            ...
            <div class="dept-card">
                <div class="dept-card-header">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h5 class="dept-title">Listado de Grados Académicos</h5>
                            <p class="dept-subtitle">Consulta, edita o elimina los registros existentes</p>
                        </div>
                    </div>
                </div>
                <div class="dept-card-body">
                    <table id="tabla-grados" class="dept-table">
                        <thead>
                            <tr>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="ri-add-circle-line"></i> Nuevo Grado Académico
                </h5>
                ...
                <div class="modal-body">
                    <div class="mb-1">
                        <label for="nombreCrear" class="form-label">
                            ...
                            Nombre del Grado <span class="req">*</span>
                        </label>
...
<div class="modal fade" id="modalEditar" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:460px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="ri-edit-2-line"></i> Editar Grado Académico
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body px-4 py-3">
                <div class="delete-warning-box">
                    ...
                    <p class="delete-msg-primary">¿Eliminar grado académico?</p>
...
(function () {
    ...
    function initDataTable() {
        tabla = $('#tabla-grados').DataTable({
            ajax: { url: '<?php echo e(route("admin.grados-academicos.listar")); ?>', dataSrc: 'data' },
            ordering: true,
            paging: false,
            info: false,
            columns: [
                ...
                },
                {
                    ...
                    render: d =>
                        ...
                        + '<button class="btn btn-action btn-action-edit btn-accion-editar" data-id="' + d.id + '" data-nombre="' + escHtml(d.nombre) + '" title="Editar grado académico"><i class="ri-pencil-fill"></i></button>'
                        + '<button class="btn btn-action btn-action-delete btn-accion-eliminar" data-id="' + d.id + '" data-nombre="' + escHtml(d.nombre) + '" title="Eliminar grado académico"><i class="ri-delete-bin-fill"></i></button>'
                        + '</div>'
                }
            ],
            language: {
    ...
    function validar(inputId, iconId, fbId) {
        ...
        if (!val)                                    return setError('El nombre del grado es obligatorio.');
    ...
    function verificarNombre(inputId, iconId, fbId, idGrado, btnId) {
        const input = document.getElementById(inputId);
        const icon  = document.getElementById(iconId);
        const fb    = document.getElementById(fbId);
        const val   = input.value.trim();
        ...
        $.ajax({
            url: '<?php echo e(route("admin.grados-academicos.verificar")); ?>',
            type: 'POST',
            data: { 
                _token: CSRF, 
                nombre: val,
            ...
            success: function(r) {
                if (r.existe) {
                    ...
                    icon.innerHTML = '<i class="ri-close-circle-fill"></i>';
                    fb.className   = 'field-feedback error';
                    fb.innerHTML   = '<i class="ri-error-warning-line"></i>Este grado académico ya existe.';
                    $(btnId).prop('disabled', true);
                } else {
                    input.classList.remove('is-invalid');
                    input.classList.add('is-valid');
    ...
    function guardar() {
        ...
        $.post('<?php echo e(route("admin.grados-academicos.guardar")); ?>', {
            _token: CSRF,
        ...
        .done(r => {
            ...
            tabla.ajax.reload();
            toast('success', r.message || 'Grado académico guardado correctamente.');
        })
    ...
    function actualizar() {
        ...
        $.ajax({
            url: '/admin/grados-academicos/' + id,
            type: 'PUT',
            data: { 
                _token: CSRF, 
        ...
        .done(r => {
            ...
            tabla.ajax.reload();
            toast('success', r.message || 'Grado académico actualizado correctamente.');
        })
    ...
    function eliminarGrado(id) {
        ...
        .done(r => {
            ...
            tabla.ajax.reload();
            toast('success', r.message || 'Grado académico eliminado correctamente.');
        })
        .fail(xhr => {
...
<?php echo $__env->make('layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp_82_12\htdocs\innova-ciencia-virtual\resources\views/admin/gradosacademicos/index.blade.php ENDPATH**/ ?>