...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (pwForm) {
        pwForm.addEventListener('submit', function (e) {
            ...
            fetch('<?php echo e(route('admin.profile.change-password')); ?>', {
                method: 'POST',
                headers: {
    ...
    if (btnSubir) {
        btnSubir.addEventListener('click', function () {
            ...
            fetch('<?php echo e(route('admin.profile.upload-foto')); ?>', {
                method: 'POST',
    ...
    if (btnSubirDoc) {
        btnSubirDoc.addEventListener('click', function() {
            ...
            fetch('<?php echo e(route("admin.profile.documento.subir")); ?>', {
                method: 'POST',
                headers: { 'Accept': 'application/json' },
                body: fd,
            })
    ...
    function verificarDocumento(accion) {
        fetch('<?php echo e(route("admin.profile.documento.verificar")); ?>', {
            method: 'POST',
            headers: {
    ...
    /* ════════════════════════════════════
       MARKETING TAB
    ════════════════════════════════════ */
    <?php if(isset($tieneMarketing) && $tieneMarketing): ?>
    ...
    const mktRouteStats        = '<?php echo e(route("admin.profile.marketing.estadisticas")); ?>';
    const mktRouteInscripciones= '<?php echo e(route("admin.profile.marketing.inscripciones")); ?>';
    const mktRouteDocumentosBase = '/admin/profile/marketing/documentos/';
    const mktRouteOfertas      = '<?php echo e(route("admin.profile.marketing.ofertas-activas")); ?>';
    ...
    ════════════════════════════════════════════════════════ */
    const mktRouteGenerarEnlace = '<?php echo e(route("admin.profile.marketing.generar-enlace")); ?>';
    const mktRoutePlanesBase    = '<?php echo e(url("admin/profile/marketing/oferta")); ?>';
    ...
    function loadInscritos() {
        ...
        fetch('<?php echo e(route("admin.profile.marketing.inscritos-comprobante")); ?>')
            .then(r => r.json())
            .then(data => {
                if (!data.success) {
    ...
    function abrirModalComprobante(inscripcionId, nombre, programa, plan) {
        ...
        fetch(`<?php echo e(url('admin/profile/marketing/inscripcion')); ?>/${inscripcionId}/cuotas`)
            .then(r => r.json())
            .then(data => {
    ...
    if (btnEnviar) {
        btnEnviar.addEventListener('click', function () {
            ...
            fetch('<?php echo e(route("admin.profile.marketing.comprobante.subir")); ?>', { method: 'POST', body: formData })
                .then(r => r.json())
                .then(data => {
...
<?php /**PATH C:\xampp_82_12\htdocs\innova-ciencia-virtual\resources\views/admin/profile/scripts.blade.php ENDPATH**/ ?>