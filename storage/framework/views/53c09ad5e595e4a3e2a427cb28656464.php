...
(function () {
    ...
    function cargarSelectores() {
        $.getJSON('<?php echo e(route("admin.estudiantes.listarDepartamentos")); ?>', function (r) {
            const opts = r.data.map(d => '<option value="' + d.id + '">' + esc(d.nombre) + '</option>').join('');
            $('#rDepto').append(opts);
            $('#editDepto').append(opts);
        });
        $.getJSON('<?php echo e(route("admin.estudiantes.listarCiudades")); ?>', function (r) {
    ...
    function initDataTable() {
        tabla = $('#tabla-estudiantes').DataTable({
            ajax: { url: '<?php echo e(route("admin.estudiantes.listar")); ?>', dataSrc: 'data' },
            ordering: true,
            responsive: true,
            autoWidth: false,
            columns: [
    ...
    function bindEvents() {
        ...
        $(document).on('click', '.est-estudio-principal.not-principal', function() {
            ...
            $.get('<?php echo e(route("admin.estudiantes.obtener", ["id" => "__ID__"])); ?>'.replace('__ID__', $('#formEditar').data('estudiante-id')))
                .done(function(r) {
    ...
    function buscarPersona() {
        ...
        $.post('<?php echo e(route("admin.estudiantes.buscarCarnet")); ?>', { _token: CSRF, carnet: carnet })
            .done(function (r) {
                if (r.encontrado) {
    ...
    function abrirModalRegistro() {
        ...
        if (personaEncontrada) {
            ...
            $.post('<?php echo e(route("admin.estudiantes.buscarCarnet")); ?>', { _token: CSRF, carnet: p.carnet })
                .done(function (r) {
                    if (r.ya_estudiante) {
    ...
    function guardarEstudiante() {
        ...
        $.ajax({
            url: '<?php echo e(route("admin.estudiantes.guardarPersona")); ?>',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false
    ...
    function registrarComoEstudiante(personaId) {
        $.post('<?php echo e(route("admin.estudiantes.registrar")); ?>', {
            _token: CSRF,
    ...
    function editarEstudiante(id) {
        $.get('<?php echo e(route("admin.estudiantes.obtener", ["id" => "__ID__"])); ?>'.replace('__ID__', id))
            .done(function (r) {
                const e = r.data;
                const p = e.persona;
    ...
    function verificarCarnetRegistro(val) {
        $.post('<?php echo e(route("admin.estudiantes.verificarCarnetPersona")); ?>', { _token: CSRF, carnet: val }, function (r) {
    ...
    function verificarCorreoRegistro(val) {
        $.post('<?php echo e(route("admin.estudiantes.verificarCorreoPersona")); ?>', { _token: CSRF, correo: val }, function (r) {
    ...
    $('#btnConfirmarCrearCuentas').on('click', function () {
        ...
        $.ajax({
            url: '<?php echo e(route("admin.estudiantes.crearCuentasBatch")); ?>',
            type: 'POST',
            data: {
                _token: CSRF,
...
<?php echo $__env->make('layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp_82_12\htdocs\innova-ciencia-virtual\resources\views/admin/estudiantes/index.blade.php ENDPATH**/ ?>