        ...
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
            ...
            <div style="display: flex; align-items: center; gap: 12px;">
                <div style="flex: 1; max-width: 150px; height: 8px; background: #e2e8f0; border-radius: 4px; overflow: hidden;">
                    <div style="height: 100%; background: linear-gradient(90deg, #fc7b04, #f97316); border-radius: 4px; width: {{ $pctDocs }}%; transition: width 0.3s;"></div>
                </div>
                <span style="font-size: 0.875rem; font-weight: 700; color: #fc7b04;">{{ $pctDocsFormatted }}%</span>
            </div>
        </div>

        {{-- SECCIÓN 1: DOCUMENTACIÓN DEL ESTUDIANTE --}}
                    ...
                    <div class="col-md-6">
                        <div class="doc-card-compact">
                            <div class="doc-header">
                                <div style="width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1rem; background: {{ $bgIcon }}; color: {{ $colorIcon }}; flex-shrink: 0;">
                                    <i class="{{ $doc['icono'] }}"></i>
                                </div>
                                ...
                                @if ($doc['archivo'])
                                    <div style="display: flex; align-items: center; gap: 10px; padding: 10px; background: #f8fafc; border-radius: 8px; margin-bottom: 10px;">
                                        ...
                                        <div style="flex: 1; min-width: 0;">
                                            ...
                                            <div style="font-size: 0.65rem; display: flex; align-items: center; gap: 4px; color: {{ $doc['verificado'] ? '#16a34a' : '#d97706' }};">
                                                @if ($doc['verificado'])
        ...
        <div>
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; flex-wrap: wrap; gap: 12px;">
                <h4 style="font-size: 1rem; font-weight: 600; color: #1e293b; display: flex; align-items: center; gap: 8px; margin: 0;">
                    ...
                    Formación Académica
                    <span class="badge" style="background: #e2e8f0; color: #475569; font-size: 0.7rem; font-weight: 600;">{{ $estudios->count() }}</span>
                </h4>
            ...
            @php $totalEstudios = $estudios->count(); @endphp
            ...
            <div class="" style="background: linear-gradient(135deg, #1a3a5c 0%, #1e5799 55%, #2e86de 100%); border-radius: 16px; padding: 1.25rem 1.5rem; display: flex; align-items: center; gap: 1.25rem; margin-bottom: 1.25rem; position: relative; overflow: hidden; flex-wrap: wrap;">
                ...
                <div style="flex: 1; min-width: 0; position: relative; z-index: 1;">
                    ...
                    <div style="font-size: 1.15rem; font-weight: 700; color: #fff; font-family: 'Outfit', sans-serif; line-height: 1.2; margin-bottom: 0.3rem;">{{ $estudioPrincipal->grado_academico?->nombre ?? '—' }}</div>
                    <div style="display: flex; flex-wrap: wrap; gap: 0.5rem 1rem; font-size: 0.78rem; color: rgba(255,255,255,0.75);">
                        <span><i class="ri-briefcase-line"></i> {{ $estudioPrincipal->profesion?->nombre ?? '—' }}</span>
                        <span><i class="ri-building-line"></i> {{ $estudioPrincipal->universidad?->nombre ?? '—' }}{{ $estudioPrincipal->universidad?->sigla ? ' (' . $estudioPrincipal->universidad->sigla . ')' : '' }}</span>
                    </div>
                </div>
                <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 0.4rem; flex-shrink: 0; position: relative; z-index: 1;">
                    ...
                    <span style="display: inline-flex; align-items: center; gap: 0.28rem; padding: 0.22rem 0.7rem; border-radius: 20px; font-size: 0.68rem; font-weight: 700; {{ $estudioPrincipal->estado === 'Concluido' ? 'background: rgba(46,154,110,0.22); color: #7eefc4; border: 1px solid rgba(46,154,110,0.38);' : 'background: rgba(251,191,36,0.2); color: #fbbf24; border: 1px solid rgba(251,191,36,0.35);' }}">
                        <i class="ri-{{ $estudioPrincipal->estado === 'Concluido' ? 'checkbox-circle-fill' : 'time-line' }}"></i>
            ...
            <div class="est-table-wrap">
                <table class="est-table">
                    ...
                    <tbody>
                        @foreach($estudios->sortByDesc('principal') as $index => $est)
                        <tr>
                            ...
                            <td style="font-size:0.84rem;">
                                ...
                                @if($est->universidad?->sigla)
                                    <span style="font-size:0.7rem;color:#64748b;"> ({{ $est->universidad->sigla }})</span>
                                @endif
                            </td>
                            <td>
                                <span class="estado-badge-est {{ $est->estado === 'Concluido' ? 'concluido' : 'en-desarrollo' }}">
            ...
            {{-- TOGGLE PANELS (one per estudio) --}}
            @foreach($estudios as $est)
            @php
                $tieneDA  = !empty($est->documento_academico);
                $tieneDP  = !empty($est->documento_provision_nacional);
            @endphp
            <div class="est-docs-panel" id="est-docs-panel-{{ $est->id }}" style="display:none;margin-top:0.75rem;border:1px solid var(--est-border);border-radius:var(--radius-md);overflow:hidden;">
                <div style="padding:0.65rem 1rem;background:linear-gradient(135deg,var(--est-primary-light),rgba(252,123,4,0.03));border-bottom:1px solid var(--est-border);display:flex;align-items:center;gap:0.5rem;">
                    ...
                    <span style="font-size:0.78rem;font-weight:700;color:var(--est-text);">Documentos — {{ $est->grado_academico?->nombre ?? 'Estudio #'.$est->id }}</span>
                </div>
                <div class="est-docs-inner">
                    ...
                    <div class="est-doc-card">
                        ...
                        <div class="est-doc-actions">
                            ...
                            <button class="est-doc-btn {{ $est->documento_academico_verificado ? 'est-doc-btn-uncheck' : 'est-doc-btn-check' }} btn-verificar-doc"
                                style="{{ $tieneDA ? 'display:flex;' : 'display:none;' }}"
                                data-id="{{ $estudiante->id }}"
                                data-tipo="documento_academico"
                                data-estudio-id="{{ $est->id }}"
                    ...
                    <div class="est-doc-card">
                        ...
                        <div class="est-doc-actions">
                            ...
                            <button class="est-doc-btn {{ $est->documento_provision_verificado ? 'est-doc-btn-uncheck' : 'est-doc-btn-check' }} btn-verificar-doc"
                                style="{{ $tieneDP ? 'display:flex;' : 'display:none;' }}"
                                data-id="{{ $estudiante->id }}"
                                data-tipo="documento_provision_nacional"
                                data-estudio-id="{{ $est->id }}"
