            ...
            <div class="dph-inner">
                <div class="dph-left">
                    ...
                    <div class="dph-text-block">
                        <h1 class="dph-title">Cargos</h1>
                        <p class="dph-desc">Gestión y administración de cargos</p>
                    </div>
                </div>
                <div class="dph-right">
                ...
                <div class="dept-card">
                    <div class="dept-card-header">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h5 class="dept-title">Listado de Cargos</h5>
                                <p class="dept-subtitle">Consulta, edita o elimina los registros existentes</p>
                            </div>
                        </div>
                    </div>
                    <div class="dept-card-body">
                        <table id="tabla-cargos" class="dept-table">
                            <thead>
                                <tr>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="ri-add-circle-line"></i> Nuevo Cargo
                    </h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-1">
                            <label for="nombreCrear" class="form-label">
                                ...
                                Nombre del Cargo <span class="req">*</span>
                            </label>
                            <div class="field-wrapper">
                                <input type="text" class="form-control" id="nombreCrear"
                                    placeholder="Ej: Director, Coordinador, Supervisor…" maxlength="100" autocomplete="off">
    ...
    <div class="modal fade" id="modalEditar" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width:460px;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="ri-edit-2-line"></i> Editar Cargo
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body px-4 py-3">
                    <div class="delete-warning-box">
                        ...
                        <p class="delete-msg-primary">¿Eliminar cargo?</p>
    ...
    <script>
        (function() {
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: d =>
                                ...
                                '" title="Editar cargo"><i class="ri-pencil-fill"></i></button>' +
                                '<button class="btn btn-action btn-action-delete btn-accion-eliminar" data-id="' +
                                d.id + '" data-nombre="' + escHtml(d.nombre) +
                                '" title="Eliminar cargo"><i class="ri-delete-bin-fill"></i></button>' +
                                '</div>'
                        }
                    ],
                    language: {
            ...
            function validar(inputId, iconId, fbId) {
                ...
                if (!val) return setError('El nombre del cargo es obligatorio.');
            ...
            function verificarNombre(inputId, iconId, fbId, idCargo, btnId) {
                const input = document.getElementById(inputId);
                const icon = document.getElementById(iconId);
                const fb = document.getElementById(fbId);
                const val = input.value.trim();
                ...
                $.ajax({
                    url: '{{ route('admin.cargos.verificar') }}',
                    type: 'POST',
                    data: {
                        _token: CSRF,
                        nombre: val,
                    ...
                    success: function(r) {
                        if (r.existe) {
                            ...
                            icon.innerHTML = '<i class="ri-close-circle-fill"></i>';
                            fb.className = 'field-feedback error';
                            fb.innerHTML = '<i class="ri-error-warning-line"></i>Este cargo ya existe.';
                            $(btnId).prop('disabled', true);
                        } else {
                            input.classList.remove('is-invalid');
                            input.classList.add('is-valid');
            ...
            function guardar() {
                ...
                $.post('{{ route('admin.cargos.guardar') }}', {
                        _token: CSRF,
                    ...
                    .done(r => {
                        ...
                        tabla.ajax.reload();
                        toast('success', r.message || 'Cargo guardado correctamente.');
                    })
            ...
            function actualizar() {
                ...
                const id = $('#idEditar').val();
                console.log('Updating cargo ID:', id);
                ...
                $.ajax({
                    ...
                    .done(r => {
                        ...
                        tabla.ajax.reload();
                        toast('success', r.message || 'Cargo actualizado correctamente.');
                    })
                    .fail(xhr => {
            ...
            function eliminarCargo(id) {
                ...
                $.ajax({
                    ...
                    .done(r => {
                        ...
                        tabla.ajax.reload();
                        toast('success', r.message || 'Cargo eliminado correctamente.');
                    })
                    .fail(xhr => {
