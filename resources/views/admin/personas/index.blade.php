...
/* ── Header con degradado y patrón decorativo ── */
.dept-page-header {
...
.btn-action-edit {
    background: rgba(252,123,4,.10) !important;
    color: #b85500 !important;
...
#modalCrear .modal-content, #modalEditar .modal-content {
    border: none !important;
    border-radius: 18px !important;
    overflow: hidden;
...
#modalCrear .modal-title, #modalEditar .modal-title {
    color: #fff !important;
    font-weight: 700 !important;
...
#modalCrear .modal-title i, #modalEditar .modal-title i {
...
#modalCrear .modal-footer, #modalEditar .modal-footer {
    background: #f8fafc;
...
.section-sep {
    color: #b85500 !important;
    border-bottom: 1.5px solid rgba(252,123,4,.18) !important;
...
.section-sep i {
    ...
    display: inline-flex; align-items: center; justify-content: center;
    border-radius: 6px; font-size: .8rem !important;
}

/* Foto de perfil — borde naranja */
.photo-upload-container { transition: all .2s; }
.photo-upload-container:hover img { border-color: #fc7b04 !important; transform: scale(1.04); box-shadow: 0 6px 18px rgba(252,123,4,.25); }
.photo-upload-container img { transition: all .2s; }

/* Inputs en modales */
#modalCrear .form-control, #modalCrear .form-select,
#modalEditar .form-control, #modalEditar .form-select {
...
#modalCrear .form-control:focus, #modalCrear .form-select:focus,
...
#modalCrear .form-label i, #modalEditar .form-label i {
    ...
    display: inline-flex; align-items: center; justify-content: center;
    border-radius: 5px; font-size: .78rem !important;
    margin-right: 2px;
}
...
/* Estudios table dentro del modal Editar — refresh suave */
.estudios-wrap { border-radius: 14px !important; border-color: rgba(252,123,4,.18) !important; }
...
html[data-bs-theme="dark"] .estudios-table thead th {
    background: rgba(252,123,4,.06) !important; border-bottom-color: rgba(252,123,4,.18) !important;
}
</style>
@endsection
        ...
        <div class="modal-content">
            ...
            <form id="formCrear" novalidate autocomplete="off" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4 text-center">
                            ...
                            <div class="photo-upload-container" style="cursor:pointer;" onclick="document.getElementById('fotografiaCrear').click()">
                                <img id="previewFotografiaCrear" src="{{ URL::asset('build/images/users/avatar-1.jpg') }}" 
                    ...
                    {{-- ─── SECCIÓN ESTUDIOS (se activa tras guardar la persona) ─── --}}
...
{{-- ════════════════ MODAL EDITAR (con estudios al fondo) ════════════════ --}}
...
(function () {
    ...
    function initDataTable() {
        tabla = $('#tabla-personas').DataTable({
            ajax: { url: '{{ route("admin.personas.listar") }}', dataSrc: 'data' },
            ordering: true,
            columns: [
                {
                    data: null,
    ...
    function verificarCarnet(inputId, iconId, fbId, personaId) {
        ...
        $.post('{{ route("admin.personas.verificarCarnet") }}', { _token: CSRF, carnet: val, id: personaId || '' }, function (r) {
    ...
    function verificarCorreo(inputId, iconId, fbId, personaId) {
        const val = document.getElementById(inputId).value.trim();
        $.post('{{ route("admin.personas.verificarCorreo") }}', { _token: CSRF, correo: val, id: personaId || '' }, function (r) {
    ...
    function guardar() {
        ...
        if (hasFoto) {
            $.ajax({
                url: '{{ route("admin.personas.guardar") }}',
                type: 'POST',
                data: payload,
                processData: false,
                contentType: false
            ...
            $.post('{{ route("admin.personas.guardar") }}', payload)
                .done(function (r) {
                    tabla.ajax.reload(null, false);
                    $('#idCrear').val(r.data.id);
                    $('#estudiosLockCrear').hide();
    ...
    function cargarEstudiosCtx(pid, ctx) {
        $.getJSON('{{ route("admin.personas.listar") }}', function (r) {
    ...
    function llenarFormEditar(d) {
        ...
        // Fotografía
        var fotoUrl = '{{ url("images/personas") }}/' + d.fotografia;
        if (d.fotografia) {
            $('#previewFotografiaEditar').attr('src', fotoUrl);
        } else {
            $('#previewFotografiaEditar').attr('src', '{{ URL::asset("build/images/user-placeholder.jpg") }}');
        }
        // Cascada departamento → ciudad
