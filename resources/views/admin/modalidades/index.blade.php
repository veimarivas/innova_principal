        ...
        <div class="dph-inner">
            <div class="dph-left">
                ...
                <div class="dph-text-block">
                    <h1 class="dph-title">Modalidades</h1>
                    <p class="dph-desc">Gestión y administración de modalidades académicas</p>
                </div>
            </div>
            <div class="dph-right">
            ...
            <div class="dept-card">
                <div class="dept-card-header">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h5 class="dept-title">Listado de Modalidades</h5>
                            <p class="dept-subtitle">Consulta, edita o elimina los registros existentes</p>
                        </div>
                    </div>
                </div>
                <div class="dept-card-body">
                    <table id="tabla-modalidades" class="dept-table">
                        <thead>
                            <tr>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="ri-add-circle-line"></i> Nueva Modalidad
                </h5>
                ...
                <div class="modal-body">
                    <div class="mb-1">
                        <label for="nombreCrear" class="form-label">
                            <i class="ri-stack-line" style="color:#fc7b04;"></i>
                            Nombre de la Modalidad <span class="req">*</span>
                        </label>
...
<div class="modal fade" id="modalEditar" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:460px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="ri-edit-2-line"></i> Editar Modalidad
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body px-4 py-3">
                <div class="delete-warning-box">
                    ...
                    <p class="delete-msg-primary">¿Eliminar modalidad?</p>
...
(function () {
    ...
    function initDataTable() {
        tabla = $('#tabla-modalidades').DataTable({
            ajax: { url: '{{ route("admin.modalidades.listar") }}', dataSrc: 'data' },
            ordering: true,
            paging: false,
            info: false,
            columns: [
                ...
                },
                {
                    ...
                    render: d =>
                        ...
                        + '<button class="btn btn-action btn-action-edit btn-accion-editar" data-id="' + d.id + '" data-nombre="' + escHtml(d.nombre) + '" title="Editar modalidad"><i class="ri-pencil-fill"></i></button>'
                        + '<button class="btn btn-action btn-action-delete btn-accion-eliminar" data-id="' + d.id + '" data-nombre="' + escHtml(d.nombre) + '" title="Eliminar modalidad"><i class="ri-delete-bin-fill"></i></button>'
                        + '</div>'
                }
            ],
            language: {
    ...
    function validar(inputId, iconId, fbId) {
        ...
        if (!val)                                    return setError('El nombre de la modalidad es obligatorio.');
    ...
    function verificarNombre(inputId, iconId, fbId, idModalidad, btnId) {
        const input = document.getElementById(inputId);
        const icon  = document.getElementById(iconId);
        const fb    = document.getElementById(fbId);
        const val   = input.value.trim();
        ...
        $.ajax({
            ...
            success: function(r) {
                if (r.existe) {
                    ...
                    icon.innerHTML = '<i class="ri-close-circle-fill"></i>';
                    fb.className   = 'field-feedback error';
                    fb.innerHTML   = '<i class="ri-error-warning-line"></i>Esta modalidad ya existe.';
                    $(btnId).prop('disabled', true);
                } else {
                    input.classList.remove('is-invalid');
                    input.classList.add('is-valid');
    ...
    function guardar() {
        ...
        $.post('{{ route("admin.modalidades.guardar") }}', {
            _token: CSRF,
        ...
        .done(r => {
            ...
            tabla.ajax.reload();
            toast('success', r.message || 'Modalidad guardada correctamente.');
        })
    ...
    function actualizar() {
        ...
        .done(r => {
            ...
            tabla.ajax.reload();
            toast('success', r.message || 'Modalidad actualizada correctamente.');
        })
    ...
    function eliminar(id) {
        ...
        .done(r => {
            ...
            tabla.ajax.reload();
            toast('success', r.message || 'Modalidad eliminada correctamente.');
        })
        .fail(xhr => {
