            ...
            <div class="dph-inner">
                <div class="dph-left">
                    ...
                    <div class="dph-text-block">
                        <h1 class="dph-title">Conceptos</h1>
                        <p class="dph-desc">Gestión y administración de conceptos de pago</p>
                    </div>
                </div>
                <div class="dph-right">
                ...
                <div class="dept-card">
                    <div class="dept-card-header">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h5 class="dept-title">Listado de Conceptos</h5>
                                <p class="dept-subtitle">Consulta, edita o elimina los registros existentes</p>
                            </div>
                        </div>
                    </div>
                    <div class="dept-card-body">
                        <table id="tabla-conceptos" class="dept-table">
                            <thead>
                                <tr>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="ri-add-circle-line"></i> Nuevo Concepto
                    </h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-1">
                            <label for="nombreCrear" class="form-label">
                                ...
                                Nombre del Concepto <span class="req">*</span>
                            </label>
                            <div class="field-wrapper">
                                <input type="text" class="form-control" id="nombreCrear"
                                    placeholder="Ej: Inscripción, Matrícula, Cuota…" maxlength="100" autocomplete="off">
    ...
    <div class="modal fade" id="modalEditar" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width:460px;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="ri-edit-2-line"></i> Editar Concepto
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body px-4 py-3">
                    <div class="delete-warning-box">
                        ...
                        <p class="delete-msg-primary">¿Eliminar concepto?</p>
    ...
    <script>
        (function() {
            ...
            function initDataTable() {
                tabla = $('#tabla-conceptos').DataTable({
                    ajax: {
                        url: '{{ route('admin.conceptos.listar') }}',
                        dataSrc: 'data'
                    },
                    ordering: true,
                    paging: false,
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: d =>
                                ...
                                '" title="Editar concepto"><i class="ri-pencil-fill"></i></button>' +
                                '<button class="btn btn-action btn-action-delete btn-accion-eliminar" data-id="' +
                                d.id + '" data-nombre="' + escHtml(d.nombre) +
                                '" title="Eliminar concepto"><i class="ri-delete-bin-fill"></i></button>' +
                                '</div>'
                        }
                    ],
                    language: {
            ...
            function verificarNombre(inputId, iconId, fbId, idConcepto, btnId) {
                ...
                $.ajax({
                    url: '{{ route('admin.conceptos.verificar') }}',
                    type: 'POST',
                    data: {
                        _token: CSRF,
                        nombre: val,
                    ...
                    success: function(r) {
                        if (r.existe) {
                            ...
                            icon.innerHTML = '<i class="ri-close-circle-fill"></i>';
                            fb.className = 'field-feedback error';
                            fb.innerHTML = '<i class="ri-error-warning-line"></i>Este concepto ya existe.';
                            $(btnId).prop('disabled', true);
                        } else {
            ...
            function guardar() {
                ...
                $.post('{{ route('admin.conceptos.guardar') }}', {
                        _token: CSRF,
                    ...
                    .done(r => {
                        ...
                        tabla.ajax.reload();
                        toast('success', r.message || 'Concepto guardado correctamente.');
                    })
            ...
            function actualizar() {
                ...
                $.ajax({
                    ...
                    .done(r => {
                        ...
                        tabla.ajax.reload();
                        toast('success', r.message || 'Concepto actualizado correctamente.');
                    })
            ...
            function eliminarConcepto(id) {
                ...
                $.ajax({
                    ...
                    .done(r => {
                        ...
                        tabla.ajax.reload();
                        toast('success', r.message || 'Concepto eliminado correctamente.');
                    })
                    .fail(xhr => {
