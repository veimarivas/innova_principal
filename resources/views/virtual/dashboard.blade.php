...
@section('script')
    ...
    <script>
        ...
        window.cargarModulosCronograma = function() {
            ...
            $(document).on('click', '.btn-ver-actividades', function() {
                ...
                $.get('/virtual/actividades/' + moduloId)
                    .done(function(r) {
                        ...
                        renderActividades(moduloId, r.contenidos, r.calificaciones, r.entregas || {}, r.archivos_subidos || {}, r.foros_participacion || {});
            ...
            function renderActividades(moduloId, contenidos, calificaciones, entregas, archivosSubidos, forosParticipacion) {
                const gradeMap = {};
                    ...
                    contenidos.forEach(function(seccion) {
                        ...
                        modulos.forEach(function(mod) {
                            ...
                            const url = mod.url ?
                                ...
                                '';
                            // Fechas de inicio y fin con hora
                            let fechasHtml = '';
                            ...
                            // 1. Fuente primaria: activity_dates (de APIs específicas)
                            let tsInicio = null, tsFin = null;
                            const ad = mod.activity_dates;
                            if (ad) {
                                if (mod.modname === 'assign') {
                                    tsInicio = ad.open  || null;
                                    tsFin    = ad.due   || ad.close || null;
                                } else if (mod.modname === 'quiz') {
                                    tsInicio = ad.open  || null;
                                    tsFin    = ad.close || null;
                                } else if (mod.modname === 'forum') {
                                    tsInicio = ad.open  || null;
                                    tsFin    = ad.close || ad.due || null;
                                }
                            }

                            // 2. Fuente alternativa/complementaria: mod.dates (array nativo de Moodle)
                            // Formato: [{label:"Opens:", timestamp:1234}, {label:"Due:", timestamp:5678}]
                            // Siempre evaluado para suplir fechas que activity_dates no trajo
                            if (Array.isArray(mod.dates) && mod.dates.length) {
                            ...
                            const strInicio = fmtTs(tsInicio);
                            const strFin    = fmtTs(tsFin);
                            ...
                            if (strInicio || strFin) {
                                ...
                                let tag = '';
                                if (vencidoGlobal)  tag = '<span class="moodle-date-tag late">Vencida</span>';
                                else if (noAbierto) tag = '<span class="moodle-date-tag future">No abierto</span>';

                                fechasHtml =
                                    (strInicio ? '<div class="moodle-date-row"><i class="ri-calendar-check-line"></i><span class="moodle-date-lbl">Inicio:</span><b>' + strInicio + '</b></div>' : '') +
                                    (strFin    ? '<div class="moodle-date-row' + (vencidoGlobal ? ' vencido' : '') + '"><i class="ri-calendar-close-line"></i><span class="moodle-date-lbl">Vencimiento:</span><b>' + strFin + '</b>' + (tag ? ' ' + tag : '') + '</div>' : '');
                            }

                            let btnRealizar = '';
                            if (['assign', 'quiz', 'forum'].includes(mod.modname) && !vencidoGlobal) {
